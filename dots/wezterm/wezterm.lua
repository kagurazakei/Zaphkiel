...
--- Variables
--- ---------------------------------------------------------------------------
local WALLPAPER_DIR = '/home/antonio/.config/wezterm/wallpapers'
...
local function cycle_wallpaper(window)
   ...
   local current_opacity = overrides.background
         and overrides.background[1]
         and overrides.background[1].opacity
      or DEFAULT_BACKGROUND_OPACITY
   local image_path = WALLPAPER_DIR
      .. '/'
      .. global.current_wallpaper_type
      .. '/'
      .. images[global.selected_wallpaper]
...
local function create_tab_bar_colors(theme)
   return {
      new_tab = { bg_color = '#11121d', fg_color = theme.dim },
...
config.launch_menu = padded_launch_menu({
   ...
   { label = ' antonio', args = { 'fish', '-c', 'antonio' } },
}, 9, { label = '-', args = { 'fish' } })
...
config.colors = {
   ...
   split = tostring(
      set_alpha(wezterm.color.get_builtin_schemes()[initial_theme.theme]['selection_bg'], 0.9)
   ),
...
-- layer.
local custom_bg = wezterm.color
   .parse(wezterm.color.get_builtin_schemes()[get_current_theme().theme]['background'])
   :lighten(0.1)
...
config.keys = {
   ...
   { key = 'e', mods = 'SUPER', action = prompt_line() },
}
...
wezterm.on('new-nu-tab', function(window, pane)
   local tab = pane:window():spawn_tab({ args = { 'nu' } })
   tab:set_title('nu')
end)
...
wezterm.on('toggle-light-mode', function(window, _)
   ...
   overrides.colors = {
      ...
      split = tostring(
         set_alpha(wezterm.color.get_builtin_schemes()[theme.theme]['selection_bg'], 0.9)
      ),
      tab_bar = create_tab_bar_colors(initial_theme),
   }
   ...
   -- __set_theme.fish
   -- function set_theme --on-variable WEZTERM_LIGHT_MODE
   --     set -l theme "Catppuccin Frappe"
   --     if test "$WEZTERM_LIGHT_MODE" = true
